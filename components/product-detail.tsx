...
export function ProductDetail({ product }: { product: Product }) {
  ...
  return <><main className="detail-page">
    ...
    <section className="detail-layout">
      ...
      <div className="detail-info"><p className="eyebrow">MELKORAA / {product.category.toUpperCase()}</p><h1>{product.name}</h1><p className="detail-price">{formatPrice(product.price)}</p><p className="detail-description">{product.description}</p>
        ...
        {product.type !== 'accessories' && <div className="detail-option"><div><b>SIZE</b><button className="size-guide" onClick={() => setSizeGuideOpen(true)}>SIZE GUIDE</button></div><div className="size-grid">{['XS', 'S', 'M', 'L', 'XL'].map((option) => <button key={option} className={size === option ? 'selected' : ''} onClick={() => setSize(option)}>{option}</button>)}</div></div>}
  ...
  </main><CartPanel /><SizeGuideModal product={product} isOpen={sizeGuideOpen} onClose={() => setSizeGuideOpen(false)} /></>
}
