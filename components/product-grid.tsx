...
export function ProductCard({ product }: { product: Product }) {
  ...
  return <article className="catalog-product-card">
    ...
    <div className="catalog-product-info"><div><h3>{product.name}</h3><p>{formatPrice(product.price)}</p></div><button className="catalog-add" aria-label={`Add ${product.name} to bag`} onClick={() => addToCart(product, { color: product.colors[0], size: product.type === 'accessories' ? 'ONE SIZE' : 'M' })}><ShoppingBag size={16} /></button></div>
